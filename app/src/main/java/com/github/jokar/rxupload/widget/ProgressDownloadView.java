...
public class ProgressDownloadView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        if (mPathWhite != null && mPathBlack != null) {
            ...
            float textX = Math.max(getPaddingLeft() - (int) (mBubbleWidth / 4.0f), mProgress
                    * mWidth / 100 - (int) (mBubbleWidth / 4.0f));
...
//                    mPaintText.setColor(getResources().getColor(R.color.red_wine));
                    textX = Math.max(getPaddingLeft() - (int) (mBubbleWidth / 3.2f), mProgress
                            * mWidth / 100 - (int) (mBubbleWidth / 3.2f));
...
//                    mPaintText.setColor(getResources().getColor(R.color.green_grass));
    ...
    private void makePathBubble() {
        ...
        Rect r = new Rect((int) (Math.max(getPaddingLeft() - width / 2,
                mProgress * mWidth / 100 - width / 2)), (int) (mHeight / 2 - height + calculateDeltaY()),
        ...
        // Bottom-right arc
        path.arcTo(new RectF(r.left + r.width() - 2 * radius, r.top + r.height() - arrowHeight - 2
                * radius, r.left + r.width(), r.top + r.height() - arrowHeight), 90, -90);
